...
class JQMScheduler extends JQW_Controller
{
	...
	/**
	 * Create jobsqueue entry for syncing hourly rates from SAPSF, taking uids as input.
	 */
	public function syncHourlyRatesFromSAPSF()
	{
		$this->logInfo('Start job queue scheduler FHC-Core-SAPSF->syncHourlyRatesFromSAPSF');

		// Generates the input for the new job
		$jobInputResult = $this->jqmschedulerlib->getHourlyRatesToSAPSF();
		...
		else
		{
			if (hasData($jobInputResult))
			{
				...
				// Add the new job to the jobs queue
				$addNewJobResult = $this->addNewJobsToQueue(
					JQMSchedulerLib::JOB_TYPE_SYNC_HOURLY_RATES_FROM_SAPSF, // job type
					$this->generateJobs( // gnerate the structure of the new job
						JobsQueueLib::STATUS_NEW,
		...
		$this->logInfo('End job queue scheduler FHC-Core-SAPSF->syncHourlyRatesFromSAPSF');
	}
